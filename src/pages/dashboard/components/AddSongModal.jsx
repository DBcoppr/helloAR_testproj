...
const AddSongModal = ({ onClose }) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.25)] 
      ...
      <div className="w-[798px] bg-white rounded-sm shadow-lg">
        <div className="flex justify-between items-center px-6 py-4 border-b-gray border-b">
          <h3 className=" text-base text-[rgba(0,0,0,0.85)] font-bold">Add Song</h3>
          <button onClick={onClose} className='text-[rgba(0,0,0,0.45)]'>
             <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" fill-opacity="0.45">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>   
             </svg>
         </button>
        </div>
        <form onSubmit={handleSubmit} className='px-5 py-9 pb-0'>
          <div className="mb-6">
        ...
        <div className='px-4 py-2.5 flex gap-2 border-t-gray border-t justify-end'>
              <button type="submit" className="px-4 py-1 bg-blue-500 text-white rounded-sm font-normal">
                 Add
              </button>
              <button type="submit" className="px-4 py-1 rounded-sm border border-[#D9D9D9] font-normal">
                 Cancel
               </button>
          </div>
      </div>
